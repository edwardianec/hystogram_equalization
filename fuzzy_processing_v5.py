...
def calculate_points(h, sum_pix, region_pixels, regions, h_borders):
	...
	for region in sorted_regions:
		...
		dots_count = 1
		if (percentage >= 90): dots_count = 6
		elif (percentage >= 80): dots_count = 5
		elif (percentage >= 70): dots_count = 4
		elif (percentage >= 60): dots_count = 3
...
def show_plot_graph(graph, member_params, member_vds,  equal_regions, image_filename="", wrt_path=""):
	max = 255
...
def show_graphs(h1, h2, member_params, member_vds, total_pixels, image_filename="", wrt_path=""):
	...
	plt.bar(list(h2.keys()), list(h2.values()),color=(0.2, 0.4, 0.6, 0.3))
...
def fuzzy_main_process(path):
	...
	src_hyst 			= build_hist(normalized_image)
	...
	fuzzy_image 		= fuzzyfication.get_fuzzy_image(normalized_image, member_params, member_vds)
