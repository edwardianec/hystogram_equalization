...
def get_member_params(h, region_stat):
	h_values = list(h.values())
	borders = histogram_borders(h)
	result = [borders[0]]
...
def fuzzy_process(path, wrt_path):
	...
	#left
	member_vds			= [0,20,50,105,150,195,225,255]
	# right
	#member_vds			= [0,55,105,135,170,210,235,255]
	region_stat 		= analyze_histogram(src_hyst, member_vds)

	member_params		= get_member_params(src_hyst,region_stat)
	#member_params		= [2, 3, 4, 5, 6, 8, 10, 255]
